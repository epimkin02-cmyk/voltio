...
export interface HeroSceneStill {
  ...
  offsetY: number;
  /**
   * Share of the canvas width the silhouette may fill, overriding
   * `FULLBLEED_FILL_X`.
   *
   * Needed because a wide subject is framed on its width, not its height: with
   * the car, `forWidth` binds on every screen, so `height` above stopped being
   * the thing that decides how large the subject reads and a pinned instance
   * could no longer be made smaller than the hero. This is that control back.
   */
  fillX?: number;
  /**
 ...
 *
 * **Retuned for a vehicle.** Artefakt's garment was roughly square, so stating a
 * height of 570 of the 800-unit screen also stated a sane width. A car is close
 * to 2.4 : 1, and the same 570 asks for ~1370 units of width against a 1440
 * artboard — the wheels would run off both edges before the height ever bound.
 * 300 puts the car at ~720 units wide, half the artboard, which leaves the
 * bracketed statements at either margin their room. `FULLBLEED_FILL_X` below is
 * the guard that makes this safe rather than merely correct today.
 */
const SUBJECT_HEIGHT = 300;
/** How far above the section's centre line the silhouette sits, frame units. */
const SUBJECT_RISE = 8;
/**
...
/**
 * The same width guard, for the full-bleed frame.
 *
 * Artefakt framed the full-bleed canvas on height alone, which was safe for a
 * subject no wider than it was tall. A car is not, and a subject that overflows
 * the *window* has no `overflow-hidden` to crop it politely — it simply runs
 * past both margins and collides with the copy standing there. Fitting whichever
 * axis binds costs nothing on the screens where the height already wins.
 *
 * 0.78 of the window, measured against the swept diagonal rather than the
 * resting box — so the car occupies about four fifths of the width at its widest
 * yaw and rather less head-on, which is the variation a turntable should have.
 */
const FULLBLEED_FILL_X = 0.78;
...
/**
 * The yaw the vehicle rests at before any turn is added.
 *
 * The GLB is modelled nose-along-negative-Z, so an untouched pivot puts the
 * camera behind the car — which is how a garment could be loaded and a vehicle
 * cannot: a rear three-quarter is the angle you photograph a car you are selling
 * FROM, and the angle you photograph one you are buying is the front. 208° swings
 * the nose towards the lens and leaves the near flank visible, so the silhouette
 * still reads as a car rather than a grille.
 *
 * Added at the same point as `SCROLL_TURN`, so the journey's own turn continues
 * from here rather than fighting it.
 */
const SUBJECT_YAW = MathUtils.degToRad(208);
...
/**
 * The subject's look, frozen from the dev tuner that produced it.
 *
 * **Voltio note.** Artefakt's subject was a garment: one near-black, fully rough,
 * untextured material, so the loader could flatten every mesh onto a single
 * physical material and be right. A vehicle cannot be treated that way — glass,
 * tyres, brake discs and lamp lenses are not the same surface as the paint, and
 * lacquering all of them alike produces a car-shaped solid rather than a car.
 * `MATERIAL_ROLES` below sorts the GLB's own material names into five roles and
 * `LOOK.roles` gives each its own physical parameters. Names not matched fall
 * back to `trim`, which is the safe, matte answer.
 *
 * What is unchanged from Artefakt, because it was measured rather than chosen:
 * the paint's iridescent film stays in the 120–360nm blue/violet band (past
 * ~450 the greens and magentas take over and it goes pink), the blue is carried
 * by the KEY LIGHT rather than the material, the environment stays well down,
 * and the paint is not pure black — at RGB 14 it matched the lattice behind it
 * and the silhouette dissolved.
 *
 * The original garment note, kept for the reasoning it records:
 *
...
/**
 * The GLB's material names, sorted into the five roles the look distinguishes.
 *
 * Matched case-insensitively as substrings against the source material's name,
 * **in order** — `glass` is tested before `light` so `Taillight_Glass` lands on
 * the lamp rather than the windscreen, and `paint` last so a name like
 * `Body_Color` cannot be claimed by an earlier, looser pattern. Anything that
 * matches nothing becomes `trim`.
 *
 * Keyed to this model's own names. A different vehicle GLB needs this list
 * re-read against its material list — it is the one part of the scene that
 * knows what it is looking at.
 */
const MATERIAL_ROLES: ReadonlyArray<readonly [RegExp, MaterialRole]> = [
  [/taillight|turn_signal|led/i, "lamp"],
  [/glass|projector/i, "glass"],
  [/tire|tyre|rubber/i, "rubber"],
  [/chrome|metal/i, "metal"],
  [/body_color|dodgerblue|ferrari_yellow|paint/i, "paint"],
] as const;

type MaterialRole = "paint" | "glass" | "lamp" | "metal" | "rubber" | "trim";

const roleFor = (name: string): MaterialRole => {
  for (const [pattern, role] of MATERIAL_ROLES) {
...
const LOOK = {
  ...
  /**
   * Per-role surfaces. The five above stay the paint's own numbers — `paint`
   * repeats them rather than referencing them so a tuner pass can move one
   * surface without moving the others.
   *
   * `glass` is transmissive rather than merely transparent: a dark cabin behind
   * real refraction is what stops the greenhouse reading as a hole cut in the
   * body. `lamp` is emissive and takes the key light's own blue, so the
   * lamps are the only lit thing on the car that is not a reflection. The
   * intensity is low on purpose: at 2.2 the tail lenses clipped to flat white
   * discs and read as headlights pointed at the reader. 0.55 keeps them as lit
   * glass — present in the silhouette, never the brightest thing in frame. `rubber` is deliberately not pure black for the same reason the
   * paint is not: against this lattice a true black tyre loses its edge.
   */
  roles: {
    paint: {
      colour: 0x070a14,
  ...
  } satisfies Record<MaterialRole, RoleSurface>,
  /**
...
export const HeroScene = ({ src, label, still }: HeroSceneProps) => {
  ...
  useEffect(() => {
    ...
    const frameSubject = () => {
      ...
      // **The turning extent, not the resting one.** `modelSize.x` is the width
      // of an untouched bounding box, and for a garment that was also the widest
      // the subject ever got. A car is two and a half times longer than it is
      // wide and it never stops turning — hover, journey and idle all feed the
      // same pivot — so the box's own width understates the silhouette by about
      // half at the three-quarter angles the page actually holds. The diagonal
      // is the radius the subject sweeps, and framing to it is what keeps the
      // car the same size on screen at every yaw instead of breathing as it
      // turns.
      const sweptWidth = Math.hypot(modelSize.x, modelSize.z);
      const forWidth =
        (sweptWidth * cssHeight) /
        (2 *
          cssWidth *
          (fullBleed ? (still?.fillX ?? FULLBLEED_FILL_X) : BOXED_FILL_X) *
          halfTan);
    ...
    const frame = (time: number) => {
      ...
      if (subject) {
        ...
        if (spinner) {
          spinner.rotation.y =
            SUBJECT_YAW + turnY + spin + baseTurn + idle * IDLE_ROTATION;
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        model.traverse((child) => {
          ...
          if (!source) return;
          // Upgrade in place, per role. Every layer the panel can reach is
          // switched on here with a non-zero value, because three compiles a
          // *different program* when one of these crosses zero — flipping that
          // at runtime would stall a frame every time a slider passed the
          // origin. That is also why `iridescence` is never exactly 0 even on
          // the rubber, and why `transmission`/`emissive` are given their
          // inert values rather than left undefined on the roles that do not
          // use them.
          const surface: RoleSurface = LOOK.roles[roleFor(source.name ?? "")];
          child.material = new MeshPhysicalMaterial({
            color: new Color(surface.colour),
            map: source.map,
            ...
            // Transmissive glass must not also be alpha-blended: two ways of
            // being see-through fight, and the cabin ends up brighter than the
