...
| Hero | `1:39` | `views/home/hero` → `Hero` | Transiett's hero: the driveway-handshake photograph full-bleed (`images/hero-handshake.jpg`; the footage loop in `HeroVideo` plays instead when `video` scenes are set), copy left on a green wash, rating card bottom-right, one promise card above it. Holds the `h1` (`1:46`) |
